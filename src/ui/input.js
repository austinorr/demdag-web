...
export const getSquaredDistance = (x1, y1, x2, y2) => {
  const dx = x2 - x1;
  const dy = y2 - y1;
  return dx * dx + dy * dy;
};

export const snapToMaxAcc = (state, gl, images) => {
  let mpos = state.mousePos;
  let snap = state.snapRadius;
  let snap2 = snap * snap;

  if ((snap > 0) & (images.length >= 3)) {
    let pctx = state.mousePos.x / gl.canvas.width;
    let pcty = state.mousePos.y / gl.canvas.height;
    let im = images[0];
